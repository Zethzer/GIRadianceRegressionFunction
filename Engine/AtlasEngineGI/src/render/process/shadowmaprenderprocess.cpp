...
void ShadowMapRenderProcess::init(const GLuint &width, const GLuint &height)
{
    ...
    m_pointlight_depth_maps_buffer.init(m_shadow_width, m_shadow_height);
    ...
    m_pointlight_depth_maps_buffer.attachTextures(depth_map_texture_datas);

    m_shader.init("shaders/depth.vert", "shaders/depth.frag", "shaders/depth.gs");

    m_out_textures.push_back(m_pointlight_depth_maps_buffer.getTexture(0));
}
...
void ShadowMapRenderProcess::process(const Quad &quad, const Scene &scene, const GLfloat &render_time, const GLboolean (&keys)[1024])
{
    ...
    for(GLuint i = 0; i < scene.numberOfPointLights(); ++i)
    {
        ...
        m_pointlight_depth_maps_buffer.bind();
